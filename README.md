...
**LiteGantt**는 외부 의존성이 전혀 없는(Zero Dependency), 초경량 순수 자바스크립트 간트 차트 라이브러리입니다.
복잡한 설정 없이 스크립트 파일 하나만으로 즉시 아름다운 반응형 타임라인 차트를 생성할 수 있습니다.

![LiteGantt Preview](https://via.placeholder.com/800x200?text=LiteGantt+Preview)
...
- **자동 무지개 색상 (Auto Rainbow)**: 데이터 개수에 맞춰 자동으로 색상 스펙트럼(Red-Violet)이 균등하게 분배됨.
- **컴팩트 디자인 (Compact Design)**: 여백을 최소화하여 한 화면에 많은 정보를 깔끔하게 표시.

## 🚀 시작하기 (Quick Start)

### 1. 설치 (Installation)
프로젝트에 `lite_gantt.js` 파일을 포함시키기만 하면 됩니다.
...
HTML에 차트를 그릴 컨테이너를 만들고, 자바스크립트로 데이터를 추가하세요.
...
- **endDate** (String, Optional): 종료 날짜. 생략 시 현재(Present)까지 진행 중인 것으로 간주하여 화살표가 표시됩니다.

### `.render()`
추가된 데이터를 바탕으로 차트를 그립니다.
- 데이터가 추가된 후 반드시 호출해야 화면에 나타납니다.

## 🎨 스타일 커스터마이징
`js/lite_gantt.js` 파일 상단의 `baseStyles` 변수에서 CSS를 직접 수정하여 디자인을 변경할 수 있습니다.
기본적으로 ID `lite-gantt-styles`로 스타일이 헤드에 주입됩니다.

---
© 2026 LiteGantt Project. Open Source.
